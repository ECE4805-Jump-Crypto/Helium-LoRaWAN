...
async function getResults(){
  ...
  var confiRes = await fetch('http://localhost:3000/simulation');
  var confiData = await confiRes.json();
  var confiLow = confiData.interval[0].toFixed(2);
  var confiHigh =  confiData.interval[1].toFixed(2);
  var pdPD = confiData.rewards.toFixed(2);
  HNT.innerHTML = `Confidence Interval:   (${confiLow},${confiHigh}) HNT/week`
  predict.innerHTML = `Prediction:   ${pdPD} HNT/week`; 
}
    ...
    return {
      ...
      properties: {
        description:
          ...
          '<p><strong>Gain: </strong>' + hotspot.gain/10 +
    ...
    return {
      ...
      properties: {
        description:
          ...
          '<p><strong>Gain: </strong>' + offhotspot.gain/10 +
